...
def generate_plotly_chart(df,memory, optimised_query):
    ...
    "# Aesthetic Visualization Guidelines\n\n"
    "## General Principles\n"
    "- Prioritize clarity and information density over decorative elements\n"
    "- Use a consistent color palette throughout all visualizations (4-7 complementary colors)\n"
    "- Apply appropriate white space between elements to reduce visual clutter\n"
    "- Ensure all text is legible with sufficient contrast against backgrounds\n"
    "- Use a single, professional font family throughout (sans-serif fonts recommended)\n"
    "- Always include proper titles, subtitles, and legends\n"
    "- Apply subtle grid lines that guide but don't distract\n\n"
    
    "## Chart-Specific Guidelines\n\n"
    "### Line Charts\n"
    "- Use thicker lines (2-3px) for better visibility\n"
    "- Apply subtle opacity to areas under lines when appropriate\n"
    "- Highlight important data points with markers\n"
    "- Include horizontal reference lines for key thresholds\n"
    "- Smooth lines appropriately for trend visualization\n\n"
    
    "### Bar Charts\n"
    "- Use value labels directly on bars when space permits\n"
    "- Sort bars by value rather than alphabetically when showing rankings\n"
    "- Use horizontal orientation for long category names\n"
    "- Apply consistent spacing between bars (40-60% of bar width)\n"
    "- Use subtle gradients or texture for improved aesthetics\n\n"
    
    "### Pie/Donut Charts\n"
    "- Limit to 5-7 slices maximum; use \"Other\" category for smaller values\n"
    "- Place labels outside the chart with connecting lines\n"
    "- Include percentage and absolute values in labels\n"
    "- Use a donut chart with space for a central KPI when appropriate\n"
    "- Sort segments clockwise from largest to smallest, starting at 12 o'clock\n\n"
    
    "### Heat Maps\n"
    "- Use intuitive color gradients (blue-to-red or single-hue progressions)\n"
    "- Include a clear color legend with value ranges\n"
    "- Apply borders between cells only when necessary\n"
    "- Use consistent cell sizes and proportions\n"
    "- Label axes clearly and include tooltips for detailed information\n\n"
    
    "## Data Representation\n"
    "- Choose appropriate scales (linear, logarithmic) based on data distribution\n"
    "- Start y-axis at zero for bar charts unless specifically inappropriate\n"
    "- Truncate axes carefully and always indicate when doing so\n"
    "- Use appropriate number formatting (K, M, B for large numbers)\n"
    "- Apply subtle animations for transitions when applicable\n"
    "- Highlight outliers and important data points\n\n"
    
    "## Responsive Design\n"
    "- Ensure visualizations scale appropriately across device sizes\n"
    "- Simplify visualizations for smaller screens\n"
    "- Convert to alternative chart types when the original becomes illegible at smaller sizes\n"
    "- Maintain touch-friendly interaction areas on mobile\n\n"
    
    "## Accessibility\n"
    "- Ensure sufficient color contrast (WCAG AA or AAA standard)\n"
    "- Never rely solely on color to convey information\n"
    "- Include alternative text descriptions for screen readers\n"
    "- Use patterns/textures in addition to colors for better distinction\n"
    "- Provide interactive elements for exploring data details\n\n"
