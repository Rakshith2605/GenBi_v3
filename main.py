...
'''
from fastapi import FastAPI, Depends, UploadFile, File, HTTPException
import session_manager  # Now using in-memory session management
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import JSONResponse
import pandas as pd
import numpy as np
...
# Initialize LLM instance using OpenAI API
llm = ChatOpenAI(temperature=0, model=OPENAI_MODEL, openai_api_key=OPENAI_API_KEY)
...
@app.post("/query")
async def process_query_endpoint(data: dict, user=Depends(verify_firebase_token)):
    """Process a user query about the uploaded data."""
    if "query" not in data:
        raise HTTPException(status_code=400, detail="Missing query in request.")

    user_query = data["query"]
    user_id = user["uid"]
    df = session_manager.session_dataframes.get(user_id)  # Retrieve DataFrame from in-memory session
    ...
    try:
        if query_type == "plot":
            ...
            agent_query = f"{user_query} Provide Python code to generate a Pandas DataFrame named `result_df`."
            agent_response = agent.run(agent_query)
